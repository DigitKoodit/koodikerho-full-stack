...
function App() {
  ...
  const handleAddNewPost = (obj) => {
    setPosts((oldState) => {
      const oldPosts = oldState.slice();
  ...
  return (
    <div className="App">
     ...
     {posts.map(post => <BlogCard headerImg={post.headerImg} title={post.title} body={post.body} />)}
    {modalOpen && <CreateBlogModal createPost={() => handleAddNewPost()} closeModal={() => setModalOpen(false)} />}
    </div>
  );
}
